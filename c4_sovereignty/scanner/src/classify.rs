...
fn build_signals() -> Vec<Signal> {
    ...
    let en_ua: &[(&str, f32)] = &[
        ...
        (r"(?i)UA-43", 1.5),
        (r"(?i)(?:re-?)?annex(?:ed|ation|ing)\s+(?:of\s+)?(?:the\s+)?crimea(?:n)?", 2.0),
...
fn build_quotation_markers() -> Vec<Regex> {
    let patterns = [
        ...
        r"(?i)[\u{ab}\u{201c}]\w*\s*(?:воссоединени|присоединени|республика\s+крым)\w*[\u{bb}\u{201d}]",
        ...
        // Ukrainian — scare quotes (allow words before key terms inside quotes)
        r"(?i)[\u{ab}\u{201c}]\w*\s*(?:возз'?єднанн|приєднанн|республіка\s+крим)\w*[\u{bb}\u{201d}]",
