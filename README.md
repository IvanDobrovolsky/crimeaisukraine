...
The SAS scores are regenerated by `python3 pipelines/llm/compute_sas.py`, which reads the raw audit JSONL, emits `data/sas_scores.json` + `data/sas_tiers.json`, and prints a ranking table to stdout under all six pre-registered weight schemes + three weight-free robustness metrics.
