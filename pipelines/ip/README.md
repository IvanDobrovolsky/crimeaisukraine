...
**Important corroborating finding from the `telecom` pipeline's live RIPE NCC probe**: of those same 9 ASNs, **8 are no longer held by their original Crimean operators**. They have been reassigned under [`ripe-733`](https://www.ripe.net/publications/docs/ripe-733) to Mobile Telecommunications Company K.S.C.P. (Kuwait's MTC), a Polish ISP, Yahoo-UK, and several individuals — none of which are Crimean entities. Only Miranda-Media (AS201776) remains at its original holder, created post-occupation in July 2014 as a Russian registration. The IP pipeline's 53% UA / 16% RU / 31% other split therefore reflects the mixed current state of the RIPE NCC registry, not a stable geographic Crimean population — and every "other" or "RU" answer is the measurable downstream effect of one registry transfer that did not receive sovereignty review. See [`pipelines/telecom`](../telecom/README.md) for the registry side of the story.
